...
pub fn generate_allowed_commands(
  ...
  let (mut config, config_paths) = read_allowed_commands_config(target, &config_directory)?;
...
fn read_allowed_commands_config(
  target: crate::platform::Target,
  config_directory: &Path,
) -> Result<(serde_json::Value, Vec<PathBuf>), crate::config::parse::ConfigError> {
  #[cfg(feature = "dx-machine-cache")]
  {
    let read =
      crate::config::machine_cache::read_project_with_machine_cache(target, config_directory)?;
    let mut config_paths = vec![read.config_path.clone()];
    if let Some((_, platform_config_path)) = &read.platform_config {
      config_paths.push(platform_config_path.clone());
    }
    return Ok((read.config, config_paths));
  }

  #[cfg(not(feature = "dx-machine-cache"))]
  {
    crate::config::parse::read_from(target, config_directory)
  }
}
...
mod tests {
  ...
  #[cfg(feature = "dx-machine-cache")]
  #[test]
  fn dx_allowed_commands_config_read_uses_project_machine_cache() {
    let _env = EnvGuard::set(
      crate::config::machine_cache::TAURI_DX_MACHINE_CACHE_ENV,
      "1",
    );
...
"#,
    ...
    let (config, config_paths) =
      read_allowed_commands_config(crate::platform::Target::current(), project.path())
        .expect("read config through allowed-command helper");

    assert_eq!(config["identifier"], "com.dx.acl");
    assert_eq!(config_paths, vec![project.path().join("tauri.conf.json")]);
    assert!(project
      .path()
      .join(".dx")
      .join("tauri")

...
mod app_config_machine_cache_tests;
mod env_nodejs;
mod env_rust;
pub mod env_system;
#[cfg(target_os = "macos")]
...
fn app_config_section_items(
  tauri_dir: &Path,
  frontend_dir: Option<&Path>,
) -> Result<Vec<SectionItem>> {
  Ok(app_config_section(tauri_dir, frontend_dir)?.1)
}

fn app_config_section(
  tauri_dir: &Path,
  frontend_dir: Option<&Path>,
) -> Result<(crate::helpers::config::ConfigMetadata, Vec<SectionItem>)> {
  let config = crate::helpers::config::get_config(Target::current(), &[], tauri_dir)?;
  let items = app::items(&config, frontend_dir);
  Ok((config, items))
}

#[cfg(all(test, feature = "dx-machine-cache"))]
pub(crate) struct AppConfigSectionSignatureForTests {
...
#[cfg(all(test, feature = "dx-machine-cache"))]
pub(crate) fn app_config_section_signature_for_tests(
  tauri_dir: &Path,
  frontend_dir: Option<&Path>,
) -> Result<AppConfigSectionSignatureForTests> {
  let (config, items) = app_config_section(tauri_dir, frontend_dir)?;
  let package_version = config.version.clone();
  let action_count = items
    .iter()
    .filter(|item| item.action.is_some() || item.action_if_err.is_some())
    .count();
  let descriptions = items
    .into_iter()
    .filter_map(|item| item.description)
    .collect();
  Ok(AppConfigSectionSignatureForTests {
    package_version,
    descriptions,
    action_count,
  })
...
pub fn command(options: Options) -> Result<()> {
  ...
  if let Some(tauri_dir) = &tauri_dir {
    if let Ok(items) = app_config_section_items(tauri_dir, frontend_dir.as_deref()) {
      app.items.extend(items);
    }
  }

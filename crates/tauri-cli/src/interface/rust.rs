...
mod cargo_metadata_machine_cache;
#[cfg(feature = "dx-machine-cache")]
mod cargo_settings_machine_cache;
mod desktop;
pub mod installation;
#[cfg(all(test, feature = "dx-machine-cache"))]
mod machine_cache_benchmark_tests;
#[cfg(all(test, feature = "dx-machine-cache"))]
mod machine_cache_tests;
pub mod manifest;
#[cfg(feature = "dx-machine-cache")]
mod workspace_machine_cache;
...
impl CargoSettings {
  ...
  fn load(dir: &Path) -> crate::Result<Self> {
    #[cfg(feature = "dx-machine-cache")]
    if cargo_settings_machine_cache::enabled() {
      if let Some(settings) = cargo_settings_machine_cache::read(dir) {
        return Ok(settings);
      }
    }

    let settings = Self::load_from_source(dir)?;

    #[cfg(feature = "dx-machine-cache")]
    if cargo_settings_machine_cache::enabled()
      && tauri_utils::config::machine_cache::machine_cache_writes_enabled()
    {
      let _ = cargo_settings_machine_cache::write(dir, &settings);
    }

    Ok(settings)
  }

  fn load_from_source(dir: &Path) -> crate::Result<Self> {
...
pub(crate) fn get_cargo_metadata(tauri_dir: &Path) -> crate::Result<CargoMetadata> {
  #[cfg(feature = "dx-machine-cache")]
  if cargo_metadata_machine_cache::enabled() {
    if let Some(metadata) = cargo_metadata_machine_cache::read(tauri_dir) {
      return Ok(metadata);
    }
  }
  ...
  if cargo_metadata_machine_cache::enabled()
    && tauri_utils::config::machine_cache::machine_cache_writes_enabled()
  {
    let _ = cargo_metadata_machine_cache::write(tauri_dir, &metadata);
  }
...
fn get_in_workspace_dependency_paths(tauri_dir: &Path) -> crate::Result<Vec<PathBuf>> {
  #[cfg(feature = "dx-machine-cache")]
  if cargo_metadata_machine_cache::enabled() {
    if let Some(projection) = cargo_metadata_machine_cache::read_projection(tauri_dir) {
      return Ok(projection.in_workspace_dependency_paths);
    }
  }
...
pub(crate) fn get_cargo_target_dir(args: &[String], tauri_dir: &Path) -> crate::Result<PathBuf> {
  let path = if let Some(target) = get_cargo_option(args, "--target-dir") {
    ...
    if cargo_metadata_machine_cache::enabled() {
      if let Some(projection) = cargo_metadata_machine_cache::read_projection(tauri_dir) {
        return Ok(projection.target_directory);
      }
    }
...
pub fn get_workspace_dir(tauri_dir: &Path) -> crate::Result<PathBuf> {
  #[cfg(feature = "dx-machine-cache")]
  if workspace_machine_cache::enabled() {
    if let Some(workspace_dir) = workspace_machine_cache::read(tauri_dir) {
      return Ok(workspace_dir);
    }
  }

  let workspace_dir = get_cargo_metadata(tauri_dir)
  ...
  if workspace_machine_cache::enabled()
    && tauri_utils::config::machine_cache::machine_cache_writes_enabled()
  {
    let _ = workspace_machine_cache::write(tauri_dir, &workspace_dir);
  }
